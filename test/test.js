...
QUnit.test('Testing calculateArea function with several sets of inputs', function (assert) {
    assert.equal(parseGpa(3, true), 3, 'Passed positive argument');
    assert.throws(function () { parseGpa(5, true); }, /GPA should be between 0 and 4/, 'Passing in value greater than 4 raises error');
    assert.throws(function () { parseGpa('', true); }, /GPA cannot be empty/, 'Passing in empty argument raises error');
    assert.throws(function () { parseGpa(null, true); }, /GPA cannot be null/, 'Passing in empty argument raises error');
});
